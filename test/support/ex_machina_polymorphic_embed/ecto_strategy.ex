defmodule Eshop.ExMachinaPolymorphicEmbed.EctoStrategy do
  @moduledoc """
  ...
  def handle_insert(%{__meta__: %{__struct__: Metadata}} = record, %{repo: repo}) do
    record
    |> cast()
    |> repo.insert!
  end

  def handle_insert(resource, options), do: ExMachina.EctoStrategy.handle_insert(resource, options)
  ...
  defp cast_assoc(assoc, assoc_key, %{__struct__: schema}) do
    case assoc do
      %{__meta__: %{__struct__: Metadata, state: :built}} ->
        cast(assoc)
