defmodule Eshop.CartAndCheckout.Services.AddProductToCart do
  ...
  defp upsert_cart_item(cart_id, product_id, diff) do
    case CartAndCheckout.CRUD.fetch_cart_item_by(cart_id, product_id) do
      {:ok, cart_item} ->
        CartAndCheckout.CRUD.update_cart_item(cart_item, %{quantity: cart_item.quantity + diff})

      {:error, :not_found} ->
        CartAndCheckout.CRUD.create_cart_item(%{cart_id: cart_id, product_id: product_id, quantity: diff})
    end
  end
end
